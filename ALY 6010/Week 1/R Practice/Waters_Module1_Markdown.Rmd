...
Loading the csv needed and viewing properties of the data set.
```{r}
...
Manipulating columns and creating new columns for analysis.  Also rearranging the columns that makes logical sense.
```{r}
...
Pulling the descriptive statistics for this data set.
```{r}
summary(lung_cap)
sd(lung_cap$Age)
sd(lung_cap$Height)
...
Creating frequency tables bucketed by gender and age buckets.
```{r}
...
Histograms for the age by gender.
```{r}
...
#Smokers analysis
Separate the smokers and non-smokers data.
```{r}
smokers <- filter(lung_cap, Smoke==TRUE)
nonsmokers <- filter(lung_cap, Smoke==FALSE)
```

Non-smokers scatterplot overlayed by smoker data.
```{r}
LCvA_smokers <- ggplot()+
...
Distributions of Age and Gender for smokers in the dataset.
```{r}
